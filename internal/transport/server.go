...
func (s *Server) handleJoin(payload []byte) *Frame {
	...
	// In all cases — first joiner or not — an invite code is required.
	// For the first joiner, the code must be the owner bootstrap token
	// generated by `fialka mailbox init`. This prevents any unauthenticated
	// client from claiming ownership by being the first to connect.
	inviteCode := ""
	if len(extra) > 0 {
	...
	if inviteCode == "" {
		if !hasOwner {
			return rejectJoin("owner bootstrap invite required — run: fialka mailbox init")
		}
	...
	var role string
	if !hasOwner {
		// Bootstrap invite must be an owner invite
		if inv.Role != "owner" {
			return rejectJoin("invalid owner bootstrap invite")
		}
		role = "owner"
	} else {
		role = inv.Role
